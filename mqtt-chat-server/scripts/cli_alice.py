...
def register_user():
    """注册用户"""
    log(Colors.YELLOW, "📝 Registering user: alice_cli")
    ...
    try:
        ...
        if response.status_code == 200 and data.get('success'):
            ...
            log(Colors.CYAN, f"   User ID: {user_id}")
        else:
...
def login_user():
    """登录"""
    log(Colors.YELLOW, "🔐 Logging in as: alice_cli")
...
def create_group():
    """创建群组"""
    global group_id
    log(Colors.YELLOW, "📦 Creating test group...")
    
    try:
        response = requests.post(
            f"{SERVER_URL}/api/groups",
            json={"name": "CLI Test Room", "description": "CLI chat room for testing"},
        ...
        if data.get('success'):
            ...
            log(Colors.GREEN, f"✅ Group created!")
            log(Colors.CYAN, f"   Group ID: {group_id}")
        else:
            log(Colors.YELLOW, "Getting existing groups...")
            get_existing_group()
    except Exception as e:
        log(Colors.RED, f"❌ Create group error: {e}")
        get_existing_group()
...
def get_existing_group():
    ...
    try:
        ...
        if groups and len(groups) > 0:
            ...
            log(Colors.RED, "❌ No groups found")
            sys.exit(1)
    except Exception as e:
        log(Colors.RED, f"❌ Get groups error: {e}")
        sys.exit(1)
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if sender != USERNAME:
            log(Colors.CYAN, f"[{sender}]: {content}")
    except:
        pass
...
def send_message(message):
    ...
    log(Colors.GREEN, f"[{USERNAME}]: {message}")

def main():
    """主函数"""
    log(Colors.CYAN, "=" * 50)
    log(Colors.CYAN, "   💬 MQTT Chat CLI - Alice")
    log(Colors.CYAN, "=" * 50)
    print()
    ...
    # 创建 MQTT 客户端
    client = mqtt_client.Client(client_id=f"alice_cli_{int(time.time())}")
    ...
    try:
        while True:
            try:
                message = input(f"{Colors.GREEN}[{USERNAME}]{Colors.NC}: ").strip()
