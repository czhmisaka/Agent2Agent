...
export function handleMessageAction(client: Client, topic: string, payload: Buffer): void {
  try {
    const message: ActionPayload = JSON.parse(payload.toString());
...
function handleRecall(
  ...
  try {
    // 验证消息存在且属于当前用户
    const message = db.prepare('SELECT * FROM messages WHERE id = ? AND sender_id = ?').get(messageId, userId) as any;
...
function broadcastMessageUpdate(groupId: string, messageId: string, flagType: string, value: boolean): void {
  const aedes = getAedes();
...
function broadcastReactionsUpdate(groupId: string, messageId: string): void {
  const db = getDatabase();
  ...
  // 按表情分组
  const reactionsByEmoji = new Map<string, { count: number; users: string[] }>();
  ...
  aedes.publish({
    ...
    if (err) console.error('❌ Broadcast reactions error:', err);
  });
}

function broadcastRecall(groupId: string, messageId: string, userId: string): void {
  const aedes = getAedes();
  ...
  aedes.publish({
    ...
    if (err) console.error('❌ Broadcast recall error:', err);
  });
}
