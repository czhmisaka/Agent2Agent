...
app.get('/api/subscriptions', authMiddleware, (req, res) => {
  ...
  try {
    ...
    res.json(subscriptions.map((sub: any) => ({
      id: sub.id,
      userId: sub.user_id,
...
app.get('/api/stats', authMiddleware, (req, res) => {
  ...
  try {
    ...
    `).get(targetUserId, targetUserId) as any;
...
// 获取指定用户的统计信息
app.get('/api/stats/:targetUserId', authMiddleware, (req, res) => {
  const db = getDatabase();
  const { userId } = (req as any).user;
